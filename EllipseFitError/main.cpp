...
extern "C" __declspec(dllexport) double EllipseFitError(int, int, uint8_t output[], uint8_t gt[]);

/// --------------------------------------------------------------------------------
/// Compute the distance between ALL detected blob pixels & the GT ellipse
/// 
double EllipseFitError(int width, int height, uint8_t output[], uint8_t gt[]) {
	...
	double elipsFitErr = ComputeEllipseError(&eq, px, py, noPixels);
	...
	return std::min(elipsFitErr, 1.0);
} //end-EllipseFitError
...
int main() {
	for (int i = 1; i < 50; i++) {
		...
		double error1 = EllipseFitError(eyeImage.cols, eyeImage.rows, eyeImage.data, gtImage.data);
		printf("i: %d, Error1: %.5lf\n", i, error1);
	} //end-for
