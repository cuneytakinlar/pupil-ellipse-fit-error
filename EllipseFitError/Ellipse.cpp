...
//which has the origin at the top-left corner.
//This is why the word "Pixel" is especially preferred instead of "Point".
//Thus, all points' y components are reflected with a simple transform.
//
//This class implements the ellipse fitting method in [1], a fitting error 
//computation method based on the method in [2] and Ramanujan II approximation 
//for the computation of ellipse perimeter.
//
//[1] A. Fitzgibbon, M. Pilu, and R.B. Fisher, “Direct Least Square Fitting of Ellipses,” 
//IEEE Trans. Pattern Anal. Machine Intelligence, vol. 21, no. 5, May 1999.
//
//[2] R. Nürnberg, Distance from a Point to an Ellipse, 2006.
//Online available at www.ma.ic.ac.uk/~rn/distance2ellipse.pdf.
//
//
//---------------------------------VERSIONS---------------------------------
//Version Date: 01-06-2012
//the ratio of the spanned angle to the total perimeter (alpha) is included
//Cihan Topal
//
//Version Date: 07.09.2012
//normalized rms fitting error per pixel (rmsError / noPoints) is included
//Cihan Topal
//
//Version Date: 23.02.2014
//fast approximations for sine and cosine added (controlled with preprocessor commands)
//Cihan Topal
...
//to fix the incompatibility between struct Pixel definitions
//#define x r
//#define y c
#define x c
#define y r
...
//Preprocessor commands to fuse fast sine & cos approximations
//#define sin FastSinHP
//#define cos FastCosHP

//Fast elips approximation - avg. error of fcn: %0.2
...
Ellipse::Ellipse(Pixel* points, int noPnts)
{
	//Initializations
	InitParams();
	...
	fitPoints = (Pixel*) malloc(noPoints * sizeof(Pixel));
	memcpy(fitPoints, points, noPoints * sizeof(Pixel));
...
Ellipse::Ellipse(double* pX, double *pY, int noPnts)
{
	//Initializations
	InitParams();
...
Ellipse::Ellipse(double coefs[6])
{
	//Initializations
	InitParams();
...
int Ellipse::GetNoPoints()
{
	return noPoints;
}

Pixel Ellipse::GetCenter()
{
	Pixel tmp;
	tmp.x = (int)cX;
	tmp.y = (int)-cY;
...
void Ellipse::GetCoefficients(double* coefs)
{
	coefs[0] = A1;
	coefs[1] = B1;
	coefs[2] = C1;
...
double Ellipse::GetSpannedAngleRatio()
{
	//if perimeter hasn't been computed, compute it!
	if (perimeter == 0)	GetPerimeter();

	return (double)noPoints / perimeter;
}
Pixel* Ellipse::DrawEllipse(int resolution)
{
	if (drawPoints != NULL)	return drawPoints;

	drawPoints = (Pixel*) malloc (resolution * sizeof(Pixel));
...
inline double Ellipse::DistanceBwPoints(Pixel p1, Pixel p2)
{
	int xDist, yDist;
	double distance;
...
inline double Ellipse::DistanceBwPoints(double x1, double y1, double x2, double y2)
{
...
inline double Ellipse::SquaredDistanceBwPoints(Pixel p1, Pixel p2)
{
	int xDist, yDist;
	double distance;
...
inline double Ellipse::SquaredDistanceBwPoints(double x1, double y1, double x2, double y2)
{
	double xDist, yDist, distance;
...
inline double Ellipse::NewtonRaphsonIteration(double theta, double x, double y)
{
...
double Ellipse::NewtonRaphsonThetaEstimation(double th0, double x, double y)
{
...
double Ellipse::GetDistance(Pixel point, double& estimation)
{
	...
	Pixel candidates[4];
	Pixel transPnt;		//translated and rotated point
	
	//translate & rotate candidate point coordinates
	transPnt.x = point.x - cX;
	transPnt.y = point.y - cY;
...
double Ellipse::GetDistance(double pX, double pY, double& estimation)
{
	//for different initial values for 4 estimations (in radians)
	double th0[4], estimations[4];
...
double Ellipse::GetSquaredDistance(Pixel point, double& estimation)
{
	//for different initial values for 4 estimations (in radians)
	double th0[4], estimations[4];
...
double Ellipse::GetSquaredDistance(double pX, double pY, double& estimation)
{
	//for different initial values for 4 estimations (in radians)
	double th0[4], estimations[4];
...
double Ellipse::GetAverageFittingError()
{
...
double Ellipse::GetRmsFittingError()
{
	...
	//total distance of fitting points to the ellipse curve
	//for(int i=0 ; i<noPoints ; i = i + 5)			//downsample arc pixels
	for(int i=0 ; i<noPoints ; i++)		
	{
		if (fitPoints != NULL)	//integer form
		{
	...
	//rmsError = sqrt(rmsError/ (noPoints / 5));	//downsample arc pixels

	return rmsError;
}

double Ellipse::GetRmsFittingError(Pixel *pixels, int noPixels) {
	double totalRmsError = 0;
	for (int i = 0; i < noPixels; i++) {
		double estimation;
		totalRmsError += GetSquaredDistance(pixels[i], estimation);
	} // end-for

	return sqrt(totalRmsError / noPixels);
} //end-GetRmsFittingError

double Ellipse::GetNormalizedRmsFitErr()
{
	if (normRmsErr != 0) return normRmsErr;

	normRmsErr = (GetRmsFittingError() / noPoints) * 100;
	//normRmsErr = (GetRmsFittingError() / (noPoints / 5)) * 100;	//downsample arc pixels

	return normRmsErr;
}

Pixel* Ellipse::GetClosestPoints()
{
	if (closePoints != NULL) return closePoints;
	closePoints = (Pixel*)malloc(noPoints*sizeof(Pixel));
...
double Ellipse::GetClosestPointAndDistance(Pixel test, Pixel &closest)
{
...
double Ellipse::GetClosestPointAndDistance(double testX, double testY, Pixel &closest)
{
	double distance, estimation;
